				...
				{TeamMembers.map((member, index) => {
					...
					return (<div key={index}  className="p-4 lg:w-1/2">
						<div className="h-full flex sm:flex-row flex-col items-center sm:justify-start justify-center text-center sm:text-left">
							{member && member.mainImage && member.mainImage.asset ? (
							<img src={imageUrlFor(buildImageObj(member.mainImage)).width(192).height(192).url()}
							alt='team member'
							...
							<div className="flex-grow sm:pl-8">
								<h2 className="title-font font-medium text-lg text-gray-900">{memberName}</h2>
								<h3 className="text-gray-500 mb-3">{member.role}</h3>
								<p className="mb-4">{member.email}<br/><span><b>NMLS :</b>{member.nmls}</span><br/>{member.phone}</p>
